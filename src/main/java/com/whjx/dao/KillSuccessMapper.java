...
public interface KillSuccessMapper {
    ...
    @Select("SELECT kill_code,user_id,kill_id,kill_status,creat_time,kill_spu_id FROM w_kill_success,timestampdiff(minute,create_time,now()) WHERE kill_status = 0")
    List<KillSuccess> selectExpirOrder();
    int countByKillId(@Param("killId") Integer killId,@Param("openId") String openId);
